...
- (void)delegateSelectedAnnotationRouteHistory:(MKAnnotationView *)view
{
    if (delegate && [delegate respondsToSelector:@selector(mapView:didSelectAnnotationRouteHistory:)])
    {
        [delegate mapView:self didSelectAnnotationRouteHistory:view];
    }
}


- (void)delegateSelectedAnnotationSendMessage:(MKAnnotationView *)view
{
    if (delegate && [delegate respondsToSelector:@selector(mapView:didSelectAnnotationSendMessage:)])
    {
        [delegate mapView:self didSelectAnnotationSendMessage:view];
    }
}
- (void)delegateSelectedAnnotationPaletteChange:(MKAnnotationView *)view withObject:(WebScriptObject *)scriptObject
{
    if (delegate && [delegate respondsToSelector:@selector(mapView:delegateSelectedAnnotationPaletteChange:withObject:)])
    {
        [delegate mapView:self delegateSelectedAnnotationPaletteChange:view withObject:scriptObject];
    }
}
