...
- (void) updateAnnotationColor:(NSString *)rgba annotation:(id < MKAnnotation >)annotation
                  scriptObject:(WebScriptObject *)scriptObject
{
    WebScriptObject *webScriptObject = [webView windowScriptObject];
//    NSLog(@"[self selectedAnnotations]:%@",[self selectedAnnotations]);
    WebScriptObject *annotationScriptObject ;
    if (scriptObject !=nil)
        annotationScriptObject = scriptObject;
    else
        annotationScriptObject= (WebScriptObject *)[annotationScriptObjects objectForKey:annotation];
    NSArray *args = [NSArray arrayWithObjects:annotationScriptObject,rgba, nil];
    [webScriptObject callWebScriptMethod:@"setColor" withArguments:args];
}
...
- (void)selectAnnotationPaletteChange:(id < MKAnnotation >)annotation
                           withObject:(WebScriptObject *)scriptObject
{
    ...
    [self delegateSelectedAnnotationPaletteChange:annotationView withObject:(WebScriptObject *)scriptObject];

}
- (void)selectAnnotationSendMessage:(id < MKAnnotation >)annotation animated:(BOOL)animated
{
    MKAnnotationView *annotationView = (id)[annotationViews objectForKey: annotation];
    [selectedAnnotations addObject:annotation];
    [self delegateSelectedAnnotationSendMessage:annotationView];

}
- (void)selectAnnotationRouteHistory:(id < MKAnnotation >)annotation animated:(BOOL)animated
{
    MKAnnotationView *annotationView = (id)[annotationViews objectForKey: annotation];
    [selectedAnnotations addObject:annotation];
    [self delegateSelectedAnnotationRouteHistory:annotationView];
