...
+ (NSString *) webScriptNameForSelector:(SEL)sel
{
    ...
    if (sel == @selector(annotationScriptObjectRouteHistoryRequested:))
    {
        name = @"annotationScriptObjectRouteHistoryRequested";
    }

    if (sel == @selector(annotationScriptObjectSendMessageRequested:))
    {
        name = @"annotationScriptObjectSendMessageRequested";
    }

    if (sel == @selector(annotationScriptObjectPaletteRequested:))
    {
        name = @"annotationScriptObjectPaletteRequested";
    }
...
+ (BOOL)isSelectorExcludedFromWebScript:(SEL)aSelector
{
    ...
    if (aSelector == @selector(annotationScriptObjectRouteHistoryRequested:))
    {
        return NO;
    }

    if (aSelector == @selector(annotationScriptObjectSendMessageRequested:))
    {
        return NO;
    }

    if (aSelector == @selector(annotationScriptObjectPaletteRequested:))
    {
        return NO;
    }
...
- (void)annotationScriptObjectRouteHistoryRequested:(WebScriptObject *)annotationScriptObject
{
    for (id <MKAnnotation> annotation in annotations)
    {
        WebScriptObject *scriptObject = (WebScriptObject *)[annotationScriptObjects objectForKey: annotation];
        if ([scriptObject isEqual:annotationScriptObject])
        {
            [self selectAnnotationRouteHistory:annotation animated:NO];
        }
    }
}

- (void)annotationScriptObjectSendMessageRequested:(WebScriptObject *)annotationScriptObject
{
    for (id <MKAnnotation> annotation in annotations)
    {
        WebScriptObject *scriptObject = (WebScriptObject *)[annotationScriptObjects objectForKey: annotation];
        if ([scriptObject isEqual:annotationScriptObject])
        {
            [self selectAnnotationSendMessage:annotation animated:NO];
        }
    }
}

- (void)annotationScriptObjectPaletteRequested:(WebScriptObject *)annotationScriptObject
{
    for (id <MKAnnotation> annotation in annotations)
    {
        WebScriptObject *scriptObject = (WebScriptObject *)[annotationScriptObjects objectForKey: annotation];
        if ([scriptObject isEqual:annotationScriptObject])
        {
            [self selectAnnotationPaletteChange:[[self selectedAnnotations] objectAtIndex:0] withObject:nil];
            break;
        }
//        [self setSelectedAnnotations:[NSArray arrayWithObject:annotationFound]];
    }
}
