...
//@property (nonatomic) 	otSocket socket;
...
- (void)connect {
    ...
    NSLog(@"SSL connection done for APNS");
}
...
- (void)disconnect {
    ...
    NSLog(@"SSL connection broken for APNS");
...
- (void)push
{
    ...
    if([self.deviceToken length] <=0 || [self.payload length]<=0) {
		return;
	}
    
    NSLog(@"Device Token:%@, Payload:%@", self.deviceToken, self.payload);
    @try {
        // Convert string into device token data.
        NSMutableData *deviceToken = [NSMutableData data];
        unsigned value;
        //    self.deviceToken = @"b63dc19a da0f9ee3 fe8143cc 39e2309a a0b0ad43 77fe03de 0db08f83 5adf075d";
        ...
        //b63dc19a da0f9ee3 fe8143cc 39e2309a a0b0ad43 77fe03de 0db08f83 5adf075d
        // Compose message.
        ...
        if (result != noErr)
            NSLog(@"Push success. SSLWrite(): %d %zd", result, processed);
        else
        {
            NSLog(@"error sending message %d",result);
        }
