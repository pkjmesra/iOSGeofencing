...
+ (void)createNewIdentity
{
	...
	**/
    ...
    CFMutableArrayRef keyUsage = (__bridge CFMutableArrayRef)[NSMutableArray arrayWithObjects:kSecAttrCanEncrypt, kSecAttrCanDecrypt,kSecAttrCanDerive,kSecAttrCanSign,kSecAttrCanVerify,kSecAttrCanWrap,kSecAttrCanUnwrap, nil];
    
    // Create and populate the key attributes array
    CFMutableArrayRef keyAttributes = (__bridge CFMutableArrayRef)[NSMutableArray arrayWithObjects:[NSNumber numberWithInt: CSSM_KEYATTR_SENSITIVE],[NSNumber numberWithInt:CSSM_KEYATTR_EXTRACTABLE], nil];

	importParameters.keyUsage = keyUsage;
	...
	while(searchRef && (SecIdentitySearchCopyNext(searchRef, &currentIdentityRef) != errSecItemNotFound))
	{
		...
		if(privateKeyRef)
		{
			...
			if(privateKeyAttributeList)
			{
				...
				if([name hasPrefix:@"GeofencingServer"])
				{
...
+ (NSString *)applicationTemporaryDirectory
{
	...
	NSString *appTempDir = [userTempDir stringByAppendingPathComponent:@"GeofencingServer"];
